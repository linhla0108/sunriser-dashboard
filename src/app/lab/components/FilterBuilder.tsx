...
export default function FilterBuilder({ open, conditions, onClose, onApply }: FilterBuilderProps) {
  const [local, setLocal] = useState<FilterCondition[]>(() =>
            ...
            {local.map((cond, i) => {
              ...
              return (
                <div key={cond.id} className="flex items-start gap-2">
                  <span className="mt-2.5 w-6 flex-shrink-0 text-center text-xs text-[#767676]">
                    {i === 0 ? 'IF' : 'AND'}
                  </span>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      updateCond(cond.id, {
                        ...
                        operator:
                          e.target.value === 'gpa' || e.target.value === 'batch'
                            ? 'gte'
                            : 'contains',
                        value: '',
                      })
                  ...
                  <select
                    value={cond.operator}
                    onChange={(e) =>
                      updateCond(cond.id, { operator: e.target.value as FilterOperator })
                    }
