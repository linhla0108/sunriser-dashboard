...
export default function FunnelStats({ applicants }: FunnelStatsProps) {
  ...
  return (
    ...
    >
      <Button variant="plain" size="plain" onClick={() => setExpanded(v => !v)} className="flex w-full items-center justify-between gap-3">
        {/* Compact funnel bar */}
        <div className="flex min-w-0 flex-1 items-center gap-3">
          <span className="flex-shrink-0 text-xs font-semibold tracking-widest text-[#6B5549] uppercase">Pipeline</span>
        ...
        <span className="flex-shrink-0 text-[#767676]">{expanded ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</span>
      </Button>
          ...
          {stages.map(s => (
            <div key={s.label} className="flex flex-col gap-1.5">
              <div className="flex items-center gap-1.5">
                <div className="h-2.5 w-2.5 flex-shrink-0 rounded-full" style={{ backgroundColor: s.color }} />
              ...
              <div className="h-1.5 overflow-hidden rounded-full bg-[#f9f9f9]">
                <div className="h-full rounded-full transition-all" style={{ backgroundColor: s.color, width: `${s.pct}%` }} />
              </div>
              <div className="flex items-baseline gap-1">
                <span className="text-base font-bold text-[#1b1b1b] tabular-nums">{s.count.toLocaleString()}</span>
