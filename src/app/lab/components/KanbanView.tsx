...
export default function KanbanView({ applicants, onOpenDetail, compareIds, onToggleCompare }: KanbanViewProps) {
      ...
      {COLUMNS.map(col => {
        ...
        return (
          <div key={col.id} className="flex w-[260px] flex-shrink-0 flex-col gap-2">
            {/* Column header */}
            <div className={`flex items-center justify-between rounded-2xl border border-[#e8e8e8] px-3 py-2.5 ${col.headerBg}`}>
              ...
              <span className={`rounded-full px-2 py-0.5 text-xs font-bold tabular-nums ${col.badgeBg} ${col.badgeText}`}>{items.length}</span>
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              {overflow > 0 && <div className="px-3 py-2 text-center text-xs text-[#767676]">+{overflow} more</div>}
              {items.length === 0 && (
                <div className="rounded-2xl border border-dashed border-[#e8e8e8] px-3 py-6 text-center text-xs text-[#767676]">No applicants</div>
              )}
            </div>
          </div>
        )
...
function KanbanCard({
  ...
  return (
    <div
      onClick={onOpen}
      className="kanban-card group relative cursor-pointer rounded-2xl bg-white p-3 transition-all hover:shadow-md"
    ...
    >
      ...
      <div className="relative z-10 flex items-center justify-between gap-2">
        ...
        <span className="text-[10px] font-semibold text-[#767676] tabular-nums">{a.gpa.toFixed(1)}</span>
      </div>

      {/* Round 1 badge */}
      {r1Badge && (
        <div className="relative z-10 mt-2 border-t border-[#f5f5f5] pt-2">
          <span className="rounded-full px-2 py-0.5 text-[10px] font-semibold" style={{ backgroundColor: r1Badge.bg, color: r1Badge.text }}>
            {a.round1Result}
          </span>
        </div>
      )}
