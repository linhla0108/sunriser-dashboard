...
describe("SavedViewsPopover — saved views list", () => {
  const views = [makeSavedView({ id: "sv1", name: "My View", view: "table" }), makeSavedView({ id: "sv2", name: "Kanban Mode", view: "kanban" })]
  ...
  it("calls onLoad and onClose when a saved view is clicked", () => {
    const onLoad = vi.fn()
    const onClose = vi.fn()
    render(<SavedViewsPopover {...defaultProps} savedViews={views} onLoad={onLoad} onClose={onClose} />)
