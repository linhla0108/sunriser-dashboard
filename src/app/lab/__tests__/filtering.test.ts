...
function filterApplicants(
  ...
  const { search = "", positionFilter = "", batchFilter = "", round1Filter = "", filterConditions = [] } = opts
...
describe("filterApplicants — filterConditions (Advanced Filter Builder)", () => {
  const cond = (field: string, operator: FilterCondition["operator"], value: string): FilterCondition => ({
    id: "test",
    field: field as FilterCondition["field"],
    operator,
    value,
  })
