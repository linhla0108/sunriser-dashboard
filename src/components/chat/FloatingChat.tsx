    ...
    <div className={`flex gap-2.5 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
      <div
        className={`mt-1 flex h-7 w-7 flex-shrink-0 items-center justify-center rounded-full ${
          isUser ? 'bg-[#FF5533]' : 'bg-[#ffdad3]'
        }`}
      >
        {isUser ? (
          <User size={12} className="text-white" />
        ) : (
          <Bot size={12} className="text-[#8d1600]" />
        )}
      </div>
      <div className={`max-w-[78%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
        <div
          className={`rounded-2xl px-3.5 py-2.5 text-[13px] ${
            isUser
              ? 'rounded-tr-sm bg-[#FF5533] text-white'
              : 'rounded-tl-sm border border-[#eeeeee] bg-white text-[#1b1b1b]'
          }`}
    ...
    <>
      ...
      {isOpen && (
        <div
          ...
          style={{
            boxShadow:
              'rgba(4, 23, 43, 0.08) 0px 0px 0px 1px, rgba(0, 0, 0, 0.18) 0px 24px 48px -12px',
            animation: 'slideUpFade 150ms ease-out',
          }}
        >
          {/* Header */}
          <div
            data-cid="chat-panel-header"
            className="flex flex-shrink-0 items-center justify-between border-b border-[#f9f9f9] px-4 py-3.5"
          >
          ...
          {contextOpen && (
            <div
              data-cid="chat-context-panel"
              className="flex-shrink-0 border-t border-[#f9f9f9] bg-[#fafafa] px-3 py-2.5"
            >
              <button
              ...
              >
                ...
                <span className="text-[10px] font-semibold tracking-widest text-[#767676] uppercase">
                  Data Context
                </span>
              </button>
              <div className="mb-2 flex items-center gap-2 rounded-xl border border-[#f0f0f0] bg-white p-2">
                ...
                <div className="min-w-0">
                  <p className="truncate text-[11px] font-medium text-[#1b1b1b]">
                    SUN.RISER 2026.xlsx
                  </p>
                  <p className="text-[10px] text-[#767676]">
                    646 rows · Batch 1 · Batch 2 · Batch 3
                  </p>
                </div>
              </div>
              <p className="mb-1.5 text-[10px] font-semibold tracking-widest text-[#767676] uppercase">
                Suggested
              </p>
          ...
          {/* Input bar */}
          <div
            data-cid="chat-input-bar"
            className="flex-shrink-0 border-t border-[#f9f9f9] px-3 pt-2 pb-3"
          >
            <div
