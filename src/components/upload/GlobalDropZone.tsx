...
export default function GlobalDropZone({ children, onAnalyze }: GlobalDropZoneProps) {
    ...
    async (file: File) => {
      ...
      try {
        ...
        if (rows === 0) {
          ...
          addToast(
            'success',
            `File parsed: ${file.name}. ${rows.toLocaleString()} rows, ${columns.length} columns.`
          )
        }
        ...
        addToast(
          'error',
          'Could not read file. Please check it is a valid spreadsheet or data file.'
        )
      }
    },
    [addToast]
  ...
  const filteredColumns = filterText
    ? allColumns.filter((c) => c.toLowerCase().includes(filterText.toLowerCase()))
    : allColumns
    ...
    <div className="relative">
      ...
      {showBackdrop && (
        <div
          ...
          onClick={(e) => {
            if (e.target === e.currentTarget && dropState === 'popup-open') closePopup()
          }}
        >
          {/* Drag hint */}
          {dropState === 'dragging' && (
            <p className="pointer-events-none mb-40 text-xl font-semibold tracking-tight text-white">
              Drop your file to analyze
            </p>
          )}
          ...
          {dropState === 'processing' && (
            <div
              ref={popupRef}
              className="mx-4 flex items-center gap-3 rounded-3xl bg-white px-5 py-4 sm:mx-0"
            >
          ...
          {dropState === 'error' && (
            <div
              ref={popupRef}
              className="mx-4 w-full max-w-[480px] rounded-3xl bg-white p-6 sm:mx-0"
              data-cid="drop-zone-popup"
            >
              <div className="flex items-start gap-3">
                <AlertCircle size={20} className="mt-0.5 flex-shrink-0 text-red-500" />
                <div className="flex-1">
                  <p className="mb-1 text-sm font-semibold text-[#1b1b1b]">
                    {errorMsg ?? 'Something went wrong.'}
                  </p>
                  <p className="text-xs text-[#767676]">Please check the file and try again.</p>
                </div>
                <button onClick={closePopup} className="text-[#767676] hover:text-[#1b1b1b]">
                  <X size={16} />
                </button>
              </div>
              <button
          ...
          {dropState === 'popup-open' && parsedFile && (
            <div
              ...
              style={{
                ...
                boxShadow:
                  'rgba(4, 23, 43, 0.08) 0px 0px 0px 1px, rgba(0, 0, 0, 0.2) 0px 24px 48px -12px',
              }}
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-[#767676]">
                    ...
                    {parsedFile.columns.length} columns detected
                  </p>
                </div>
                <button
                  onClick={closePopup}
              ...
              {/* Columns section */}
              <div>
                <p className="mb-2.5 text-[10px] font-semibold tracking-widest text-[#767676] uppercase">
                  Columns Detected
                </p>

                {filteredColumns.length === 0 && filterText && (
                  <p className="mb-2 text-xs text-[#767676]">
                    No columns match. Press <span className="font-semibold">[+]</span> to add as
                    custom.
                  </p>
                )}
                  ...
                  {visibleColumns.map((col) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {isCustom && (
                          <button
                            onClick={() => handleRemoveCustom(col)}
