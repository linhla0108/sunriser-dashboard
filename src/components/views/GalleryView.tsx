...
export function GalleryView({ data, onReorder, onViewDetail, onUpdateApplicant, searchQuery }: GalleryViewProps) {
      ...
      <SortableContext items={data.map(item => item.id)} strategy={rectSortingStrategy}>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {data.slice(0, 120).map(item => (
            <GalleryCard key={item.id} applicant={item} onViewDetail={onViewDetail} onUpdateApplicant={onUpdateApplicant} searchQuery={searchQuery} />
          ))}
        </div>
        {data.length > 120 && (
