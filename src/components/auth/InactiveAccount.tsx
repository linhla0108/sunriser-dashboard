...
export function InactiveAccount() {
  ...
  return (
    <main className="grid min-h-dvh place-items-center bg-background px-4">
      <div className="w-full max-w-md space-y-4 rounded-3xl border border-foreground/10 bg-white p-8 text-center shadow-sm">
        <div className="mx-auto grid size-12 place-items-center rounded-full bg-amber-100 text-amber-700">
          <ShieldAlert className="size-6" aria-hidden />
        </div>
        <h1 className="text-h1 font-semibold text-foreground">Account inactive</h1>
        <p className="text-muted-foreground text-sm leading-6">
          Your access to SUN.RISER is currently disabled
          {user?.email ? <> for <span className="font-medium text-foreground">{user.email}</span></> : null}.
