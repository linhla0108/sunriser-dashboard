  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger className="contents">
        ...
        >
          <p data-v2-muted="" className="font-semibold tracking-widest text-[#6B5549] uppercase" style={{ fontSize: "var(--text-label, 11px)" }}>
            {title}
          </p>
          <span
      ...
      <ContextMenuContent className="w-56">
        <ContextMenuGroup>
          ...
          <ContextMenuItem onClick={() => navigator.clipboard.writeText(`${title}: ${value}${subtitle ? ` (${subtitle})` : ""}`)}>
            <Copy />
            Copy metric
          </ContextMenuItem>
          <ContextMenuItem onClick={() => (window.location.href = "/candidates")}>
            <ExternalLink />
            Open candidate list
          </ContextMenuItem>
        </ContextMenuGroup>
