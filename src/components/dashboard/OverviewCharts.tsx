  ...
  return (
    <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
      ...
      <ChartCard title="Applications by Position" dataCid="chart-by-position">
        <ResponsiveContainer width="100%" height={220} className="h-[160px] sm:h-[200px] lg:h-[220px]">
          <BarChart data={positionBreakdown} margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
            <XAxis dataKey="short" tick={{ fontSize: 11, fill: "var(--muted-foreground, #6B5549)" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground, #6B5549)" }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={tooltipStyle} cursor={{ fill: "var(--muted, #f9f9f9)" }} formatter={v => [`${v ?? ""}`, "Applicants"]} />
      ...
      <ChartCard title="Round 1 Results" dataCid="chart-round1">
        <ResponsiveContainer width="100%" height={220} className="h-[160px] sm:h-[200px] lg:h-[220px]">
          <PieChart>
            <Pie data={round1Data} cx="50%" cy="45%" innerRadius={55} outerRadius={85} paddingAngle={3} dataKey="value">
              {round1Data.map((_, i) => (
      ...
      <ChartCard title="Applications by Batch" dataCid="chart-by-batch">
        <ResponsiveContainer width="100%" height={220} className="h-[160px] sm:h-[200px] lg:h-[220px]">
          <BarChart data={batchData} margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} vertical={false} />
            <XAxis dataKey="name" tick={{ fontSize: 12, fill: "var(--muted-foreground, #6B5549)" }} axisLine={false} tickLine={false} />
            <YAxis tick={{ fontSize: 11, fill: "var(--muted-foreground, #6B5549)" }} axisLine={false} tickLine={false} />
            <Tooltip contentStyle={tooltipStyle} cursor={{ fill: "var(--muted, #f9f9f9)" }} formatter={v => [`${v ?? ""}`, "Applicants"]} />
      ...
      <ChartCard title="Discovery Channels" dataCid="chart-discovery">
        <ResponsiveContainer width="100%" height={220} className="h-[160px] sm:h-[200px] lg:h-[220px]">
          <BarChart data={discoveryChannelBreakdown} layout="vertical" margin={{ top: 0, right: 20, left: 0, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke={CHART_GRID} horizontal={false} />
            <XAxis type="number" tick={{ fontSize: 11, fill: "var(--muted-foreground, #6B5549)" }} axisLine={false} tickLine={false} />
            <YAxis
              type="category"
              dataKey="channel"
