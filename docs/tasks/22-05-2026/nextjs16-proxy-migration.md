...
**Root cause 3 — provider nesting:** `ThemeProvider` calls `useAuth()` to hydrate theme settings from the user's DB profile. It was placed _outside_ `AuthProvider` in `layout.tsx`, so `AuthContext` was null and the guard threw. Fixed by swapping the nesting order: `AuthProvider` now wraps `ThemeProvider`.
