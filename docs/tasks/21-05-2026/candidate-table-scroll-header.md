...
Acceptance criteria:
- [ ] Scrolling `/candidates` keeps the workspace header visible at the top.
- [ ] The sticky header still works with the pinned toolbar visible on `/candidates`.
- [ ] Other workspace routes keep normal page scrolling.

Verification:
- [ ] Manual browser check on `/candidates?view=table`.
- [ ] Manual browser check on one non-candidates workspace route.
- [ ] Existing `TopBar` and `WorkspaceShell` tests still pass.
...
Acceptance criteria:
- [ ] In `/candidates?view=table`, table rows scroll inside the table card instead of pushing the whole page endlessly.
- [ ] The table header stays visible while the table body scrolls.
- [ ] Horizontal table scrolling still works on small widths.
- [ ] Drag, sort, inline chips, pin, detail view, and pagination controls remain usable.

Verification:
- [ ] Focused table tests pass.
...
Acceptance criteria:
- [ ] The table pagination summary remains readable above the pill nav.
- [ ] The last visible table rows and empty-state content are not hidden behind the pill nav.
- [ ] Mobile clearance accounts for the nav being at `bottom-20`; desktop clearance accounts for `sm:bottom-6`.
...
## Checkpoint
- [ ] `npm test -- ApplicantTable TableView ViewPillNav TopBar WorkspaceShell`
- [ ] `npx tsc --noEmit`
- [ ] `npm run lint`
...
Implementation has not started. This file records the plan for the `/candidates` table-scroll and sticky-header fix.
